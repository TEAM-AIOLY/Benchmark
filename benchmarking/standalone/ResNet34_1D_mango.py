...
from src.net import ResNet34_1D
...
def make_model(arch_params, dropout, spec_dims, y_dim, mean, std, device):
    ...
    model = ResNet34_1D(in_channel=1, out_dims=y_dim, mean=mean, std=std, dropout=dropout, inplanes=arch_params['NF']).to(device)
    return model
