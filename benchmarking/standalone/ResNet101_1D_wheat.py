...
from src.net import ResNet101_1D
...
def make_model(arch_params, dropout, spec_dims, y_dim, mean, std, device):
    """Create the exact ResNet 1D factory instance."""
    model = ResNet101_1D(in_channel=1, out_dims=y_dim, mean=mean, std=std, dropout=dropout, inplanes=arch_params['NF']).to(device)
    return model
...
def objective_architecture(trial, data, mean, std, spec_dims, y_dim, device, search_dir):
    ...
    arch_params = {
        'NF': trial.suggest_categorical('NF', [4, 8, 16, 32]),
    }
